...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$params = (Get-ChildItem function:\Find-Backup).Parameters.Keys
...
Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Path validation" {
        { Find-Backup -Path 'funnypath' -BackupFileExtension 'bak' -RetentionPeriod '0d' -EnableException } | Should Throw "not found"
    }
    Context "RetentionPeriod validation" {
        { Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod 'ad' -EnableException } | Should Throw "format invalid"
        { Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '11y' -EnableException } | Should Throw "units invalid"
    }
    Context "BackupFileExtension validation" {
        { Find-Backup -Path $testPath -BackupFileExtension '.bak' -RetentionPeriod '0d' -EnableException } | Should Not Throw
    }
    Context "BackupFileExtension message validation" {
        $warnmessage = Find-Backup -Path $testPath -BackupFileExtension '.bak' -RetentionPeriod '0d' 3>&1
    ...
    Context "Files found match the proper retention" {
        ...
        It "Should find all files with retention 0d" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '0d'
            $results.Length | Should Be 20
        }
        It "Should find no files '*hours*' with retention 11h" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '11h'
        ...
        It "Should find no files '*days*' with retention 6d" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '6d'
        ...
        It "Should find no files '*weeks*' with retention 6w" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '6w'
        ...
        It "Should find no files '*months*' with retention 6m" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '6m'
    ...
    Context "Files found match the proper archive bit" {
        ...
        It "Should find only files with the archive bit not set" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'bak' -RetentionPeriod '0d' -CheckArchiveBit
            $results.Length | Should Be 5
    ...
    Context "Files found match the proper extension" {
        ...
        It "Should find 5 files with extension trn" {
            $results = Find-Backup -Path $testPath -BackupFileExtension 'trn' -RetentionPeriod '0d'
            $results.Length | Should Be 5
        }
