...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    BeforeAll {
        ...
        $db2 = Get-Database -SqlInstance $script:instance2 -Database tempdb
    ...
    It "copies the table data" {
        $null = Copy-TableData -SqlInstance $script:instance1 -Database tempdb -Table dbatoolsci_example -DestinationTable dbatoolsci_example2
    ...
    It "copies the table data to another instance" {
        $null = Copy-TableData -SqlInstance $script:instance1 -Destination $script:instance2 -Database tempdb -Table dbatoolsci_example -DestinationTable dbatoolsci_example3
        $table1count = $db.Query("select id from dbo.dbatoolsci_example")
    ...
    It "supports piping" {
        $results = Get-Table -SqlInstance $script:instance1 -Database tempdb -Table dbatoolsci_example | Copy-TableData -DestinationTable dbatoolsci_example2
    ...
    It "supports piping more than one table" {
        $results = Get-Table -SqlInstance $script:instance1 -Database tempdb -Table dbatoolsci_example2, dbatoolsci_example | Copy-TableData -DestinationTable dbatoolsci_example2
        $results.Count -eq 2
    }
}