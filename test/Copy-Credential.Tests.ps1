...
if (-not $env:appveyor) {
    Describe "$CommandName Integration Tests" -Tags "IntegrationTests" {
        ...
        AfterAll {
            try {
                (Get-Credential -SqlInstance $script:instance1 -CredentialIdentity thor -ErrorAction SilentlyContinue -WarningAction SilentlyContinue).Drop()
                (Get-Credential -SqlInstance $script:instance2 -CredentialIdentity thor -ErrorAction SilentlyContinue -WarningAction SilentlyContinue).Drop()
            }
            catch { }
        ...
        Context "Copy Credential with the same properties." {
            It "Should copy successfully" {
                $results = Copy-Credential -Source $script:instance1 -Destination $script:instance2 -CredentialIdentity thorcred
                $results.Status | Should Be "Successful"
            }

            It "Should retain its same properties" {

                $Credential1 = Get-Credential -SqlInstance $script:instance1 -CredentialIdentity thor -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
                $Credential2 = Get-Credential -SqlInstance $script:instance2 -CredentialIdentity thor -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
