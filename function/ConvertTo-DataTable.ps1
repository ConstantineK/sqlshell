...
#>
  ...
  begin {
    ...
    Test-Deprecation -DeprecatedOn 1.0.0 -Alias Out-DataTable
    ...
    function Convert-Type {
      ...
      param (
        ...
        if (
          ($type -eq 'System.TimeSpan') -or
          ($type -eq 'Sqlcollaborative.Dbatools.Utility.DbaTimeSpan') -or
          ($type -eq 'Sqlcollaborative.Dbatools.Utility.DbaTimeSpanPretty')
        ) {
            $special = $true
          ...
          if ($propValueLength -gt 0) {
            ...
            if ($property.Name -in $specialColumns) {
              $Type = @{
                ...
                Type = $specialColumnsType[$property.Name]
                SizeType = $SizeType
                TimeSpanType = $TimeSpanType
              }
              $datarow.Item($property.Name) = Convert-SpecialType @Type
            }
            else {
