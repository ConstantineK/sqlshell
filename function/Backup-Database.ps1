function Backup-Database {
  ...
  .EXAMPLE
    Backup-Database -SqlInstance sql2016 -AzureBaseUrl https://az.blob.core.windows.net/azbackups/ -AzureCredential dbatoolscred -Type Full -CreateFolder

    Performs a full backup of all databases on the sql2016 instance to their own containers under the https://az.blob.core.windows.net/azbackups/ container on Azure blog storage using the sql credential "dbatoolscred" registered on the sql2016 instance.
  #>
  ...
  begin {
    if ($SqlInstance.length -ne 0) {
      ...
      if ($Database) {
        ...
        Where-Object { $_.Name -in $Database }
      }
      else {
        $DatabaseCollection = $Server.Databases |
      ...
      if ($ExcludeDatabase) {
        ...
        Where-Object  { $_.Name -notin $ExcludeDatabase }
      }
      ...
      if (
          ($MaxTransferSize % 64kb) -ne 0 -or
          $MaxTransferSize -gt 4mb
      ) {
  ...
  process {
    ...
    ForEach ($Database in $databasecollection) {
      ...
      if ($CompressBackup) {
        if (
            $Server.Edition -like 'Express*' -or
            (
                $Server.VersionMajor -eq 10 -and
                $Server.VersionMinor -eq 0 -and
                $Server.Edition -notlike '*enterprise*'
            ) -or
            $Server.VersionMajor -lt 10
          ) {
          ...
          if ($CreateFolder) {
            ...
            if ($Pscmdlet.ShouldProcess($Server.Name, "Creating folder $path")) {
              if (
                  (
                    (
                        New-SqlDirectory -SqlInstance $Server -SqlCredential $SqlCredential -Path $path
                    ).Created -eq $false
                  ) -and '' -eq $AzureBaseUrl
              ) {
      ...
      if (!$failures) {
        ...
        $percent = [Microsoft.SqlServer.Management.Smo.PercentCompleteEventHandler] {
          $Progress = @{
            id = 1
            activity = "Backing up database $dbname to $backupfile"
            percentcomplete = $_.Percent
            status = ([System.String]::Format("Progress: {0} %", $_.Percent))
          }
        ...
        Write-Progress @StartingProgress

        try {
          if ($Pscmdlet.ShouldProcess($Server.Name, "Backing up $dbname to $backupfile")) {
            $Filelist = @()

            $FileList += $Server.Databases[$dbname].FileGroups.Files |
              Select-Object @{
                Name = "FileType";
                Expression = { "D" }
              }, @{
            ...
            $FileList += $Server.Databases[$dbname].LogFiles |
              Select-Object @{
                Name = "FileType";
                Expression = { "L" }
              }, @{
            ...
            if ($Server.VersionMajor -eq '8') {
              ...
              $HeaderInfo = Get-BackupHistory -SqlInstance $Server -Database $dbname -Last -IncludeCopyOnly |
                Sort-Object -Property End -Descending |
            ...
            if ($Verify) {
              $verifiedresult = [PSCustomObject]@{
                ...
                DestinationFilePrefix = DbaVerifyOnly
              } # This was altered and might break, but I think is actually a bug in dbatool
              Restore-Database @verifiedresult
          ...
          if (
            $NoRecovery -and
            ($_.Exception.InnerException.InnerException.InnerException -like '*cannot be opened. It is in the middle of a restore.')
          ) {
            rite-Message -Message "Exception thrown by db going into restoring mode due to recovery" -Level Verbose
          }
          else {
